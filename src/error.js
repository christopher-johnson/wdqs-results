...
var root = module.exports = function(wdqsr) {
	...
	var getTryBtn = function() {
		...
		if (options.tryQueryLink) {
			...
			$tryBtn = $('<button>', {
					class: 'wdqsr_btn wdqsr_tryQuery'
				})
	...
	var draw = function() {
		var error = wdqsr.results.getException();
	...
	var canHandleResults = function(wdqsr) {
		return wdqsr.results.getException() || false;
	};
