...
var root = module.exports = function(wdqsr) {
	...
	return {
		...
		canHandleResults: function(wdqsr) {
			var results, variables;
			return (results = wdqsr.results) != null && (variables = results.getVariables()) && variables.length > 0;
		},
		getDownloadInfo: function() {
			if (!wdqsr.results) return null;
			var svgEl = wdqsr.resultsContainer.find('svg');
			if (svgEl.length > 0) {
				return {
					getContent: function() {
			...
			var $table = wdqsr.resultsContainer.find('.google-visualization-table-table');
			if ($table.length > 0) {
				return {
					getContent: function() {
