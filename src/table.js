 ...
 * @class WDQSR.plugins.table
 * @return wdqsr-table (doc)
 *
 */
var root = module.exports = function(wdqsr) {
    ...
    var tableLengthPersistencyId = (options.persistency ? wdqsr.getPersistencyId(options.persistency.tableLength) : null);

    var getRows = function() {
        var rows = [];
        var bindings = wdqsr.results.getBindings();
            ...
            for (var colId = 0; colId < vars.length; colId++) {
                ...
                if (sparqlVar in binding) {
                    if (options.getCellContent) {
                        row.push(options.getCellContent(wdqsr, plugin, binding, sparqlVar, {
                            'rowId': rowId,
                            'colId': colId,
    ...
    var eventId = wdqsr.getPersistencyId('eventId') || "wdqsr_" + $(wdqsr.container).closest('[id]').attr('id');
    var addEvents = function() {
        ...
        if (tableLengthPersistencyId) {
            table.on('length.dt', function(e, settings, len) {
                Storage.storage.set(tableLengthPersistencyId, len, "month");
            });
        }
    ...
    plugin.draw = function() {
        ...
        $(wdqsr.resultsContainer).html(table);

        var dataTableConfig = options.datatable;
        dataTableConfig.data = getRows();
        dataTableConfig.columns = options.getColumns(wdqsr, plugin);

        //fetch stored datatables length value
        var pLength = Storage.storage.get(tableLengthPersistencyId);
    ...
    plugin.getDownloadInfo = function() {
        ...
        return {
            getContent: function() {
                return require("./bindingsToCsv.js")(wdqsr.results.getAsJson());
            },
...
var formatLiteral = function(wdqsr, plugin, literalBinding) {
...
var getCellContent = function(wdqsr, plugin, bindings, sparqlVar, context) {
    ...
    if (binding.type == "uri") {
        ...
        if (plugin.options.mergeLabelsWithUris) {
            ...
            if (bindings[sparqlVar + postFix]) {
                visibleString = formatLiteral(wdqsr, plugin, bindings[sparqlVar + postFix]);
                title = href;
            }
        }
        value = "<a " + (title ? "title='" + href + "' " : "") + "class='uri' target='_blank' href='" + href + "'>" + visibleString + "</a>";
    } else {
        value = "<span class='nonUri'>" + formatLiteral(wdqsr, plugin, binding) + "</span>";
    }
 ...
 *
 * @type object
 * @attribute WDQSR.plugins.table.defaults
 */
root.defaults = {
     ...
     * @type function
     * @return string
     * @default WDQSR.plugins.table.getFormattedValueFromBinding
     */
    ...
    getColumns: function(wdqsr, plugin) {
        var includeVariable = function(variableToCheck) {
            ...
            if (variableToCheck.indexOf(postFix, variableToCheck.length - postFix.length) !== -1) {
                ...
                if (wdqsr.results.getVariables().indexOf(variableToCheck.substring(0, variableToCheck.length - postFix.length)) >= 0) {
        ...
        wdqsr.results.getVariables().forEach(function(variable) {
            cols.push({
...
root.version = {
    "WDQSR-table": require("../package.json").version,
    "jquery": $.fn.jquery,
